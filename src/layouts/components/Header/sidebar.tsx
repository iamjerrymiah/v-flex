...
export default function Sidebar({ onClose, setFilter }: { isOpen: boolean; onClose: ()=>void; setFilter?:any }) {
    ...
    return (
        <Box mt={6} overflowY="scroll" className='scroll-custom'>
            {history.length > 1 && (
                <Button
            ...
            <AnimatePresence mode="wait">
                ...
                >
                    <VStack align="start" spacing={4} mt={4} px={3}>

                        {history[history?.length - 1]?.currentCategory && (
                            <Box 
                              cursor={'pointer'} 
                              onClick={() => { 
                                  setFilter((prev:any) => ({ ...prev, categoryId: history[history.length - 1]?.currentCategory?._id }));
                                  onClose(); 
                              }}
                              // onClick={() => { navigate(`/products/vl/${history[history.length - 1]?.currentCategory?.slug ?? ""}?componentsVfcategory=${history[history.length - 1]?.currentCategory?._id}`); onClose() }}
                            >
                            ...
                            <VStack key={category._id} w="full" alignItems="stretch">
								...
								>
									...
									<Box 
										cursor={'pointer'} 
										onClick={() => { 
											setFilter((prev:any) => ({ ...prev, categoryId: category._id }));
											onClose(); 
										}}
										// onClick={() => { navigate(`/products/vl/${fullCategoryPath}?componentsVfcategory=${category._id}`); onClose(); } } 
									>
